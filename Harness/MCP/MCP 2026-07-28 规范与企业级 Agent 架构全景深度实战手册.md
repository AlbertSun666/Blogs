...
设计要点：`_meta` 的 key 采用 `io.modelcontextprotocol/xxx` 的反域名命名空间，避免与业务参数撞名，也为第三方扩展预留了空间（自定义 key 使用自己的域名前缀即可）。其中 `clientCapabilities.experimental` 是非标实验特性的声明通道，上例的 `custom_ui` 即属此类——它的协商、降级与生命周期治理在 5.6 节有完整拆解。v1 中靠握手协商一次、Session 内隐式生效的信息，现在被显式摊平到每个请求上——这正是“请求自包含”的物理实现。
...
与 5.7 节的 `tools/list_changed` 通知组合起来，完整的缓存策略是：**TTL 内用缓存 → TTL 到期或收到变更通知才重新拉取 → 拉取结果因确定性排序而不破坏 Prompt Cache**。

---

### 1.8 官方发布解读（三）：Extensions 扩展框架与首批“官方一等公民扩展”

2026-07-28 正式确立了 **Extensions（扩展）框架**，并同步发布了**第一批官方一等公民扩展 (Official Extensions)**：Tasks、MCP Apps、Enterprise Managed Authorization (EMA)。这是 MCP 治理模式的一次定型，值得先讲清“官方扩展”这个概念本身，再逐一拆解三个扩展。

#### 什么是“官方一等公民扩展”？

- **命名空间确权**：官方扩展占用 `io.modelcontextprotocol/*` 保留命名空间（如 `io.modelcontextprotocol/tasks`），第三方扩展只能使用自有域名前缀——从命名上即可区分“官方标准”与“私有约定”；
- **SEP 治理流程**：每个扩展对应一份 SEP（Specification Enhancement Proposal）提案，经社区评审后定稿，源码与规范托管在独立的 `ext-*` 仓库（如 EMA 的 `ext-auth`），拥有独立的工作组（Interest Group）与演进节奏；
- **可选协商，非强制**：扩展不进入协议核心，客户端/服务端通过能力声明（`_meta` 的 capabilities，参见 5.6 节 experimental 通道的演进路径）按需协商启用，不支持的端照常工作——**核心小而不失功能，扩展强而不绑架核心**；
- **独立版本化**：扩展随自身 SEP 演进，与核心协议的 12 个月废弃政策（2.4 节）解耦，可以更快迭代。

这一框架的设计哲学是：**核心协议 = 无状态 RPC + 三大原语；其余一切（任务、应用、企业授权）皆扩展**。评估第三方 MCP 组件时，看清它依赖的是核心能力还是某个扩展，直接决定你的客户端兼容性矩阵。

#### 扩展一：Tasks 长任务扩展（SEP-2663，AWS 贡献）

**解决什么问题**：同步 `tools/call` 必须在一个 HTTP 请求内算完，而真实业务充满分钟级甚至小时级任务（批量数据处理、报表生成、模型批推理）。v1 时代只能靠“工具先返回任务 ID、业务方自定义轮询接口”的私有协议，每个 Server 各搞一套。

**机制**：Tasks 从实验核心移出、以 `io.modelcontextprotocol/tasks` 扩展正式发布——工具调用可以返回一个**任务句柄**而非最终结果，客户端通过轮询式 `tasks/get` 查询进度，服务端经 `tasks/update` 推送增量状态；任务变更通知不再走旧的 HTTP GET 端点，而是复用 5.7 节那条 `subscriptions/listen` 单流（客户端按通知类型订阅）。

```
[ Client ] ──► tools/call generate_report ──► [ Server ] 立即返回 { taskId, status: "running" }
[ Client ] ──► tasks/get (taskId) ──────────► [ Server ] { status: "running", progress: 0.4 }
                ...（轮询或等 subscriptions/listen 通知）...
[ Client ] ◄── notifications: 任务完成 ─────── [ Server ]（经 listen 单流推送）
[ Client ] ──► tasks/get (taskId) ──────────► [ Server ] { status: "completed", result: {...} }
```

它从协议层面终结了 9.5 节的“网关 30 秒超时”难题：长任务的标准范式是**异步提交 + Tasks 轮询/订阅**，而不是硬扛同步连接。注意任务句柄正是 1.9 节“显式状态句柄”模式的官方实现——任务状态存服务端外置存储，任何节点都能响应 `tasks/get`。

#### 扩展二：MCP Apps 交互界面扩展（SEP-1865，首个官方扩展）

**解决什么问题**：工具结果只能以文本/JSON 形态回到聊天流，数据看板、可视化图表、复杂表单这类“一图胜千言”的场景表达力不足。

**机制**：Server 将 HTML/JS 界面注册为 `ui://` 协议的 Resource，工具经 `meta.ui.resourceUri` 挂链；宿主在沙箱 iframe 中渲染，界面经 `postMessage` + JSON-RPC 由宿主中继与 Server 双向通信。它由社区 MCP-UI 项目联合 OpenAI、Anthropic 共同推进，是**历史上第一个从 experimental 通道走完“实验 → 官方扩展”完整路径的样板**。声明、渲染、通信三段实现逻辑与安全模型已在 5.6 节完整拆解，此处不再重复。

#### 扩展三：Enterprise Managed Authorization (EMA) 企业托管授权（SEP-990）

**解决什么问题**——“授权同意税 (Consent Tax)”：标准 OAuth 流程要求**每个员工对要用的每个 MCP Server 各点一次授权**。12 个 Server × 100 名员工 = 1200 次授权弹窗，外加同量级的 IT 支持工单。这是企业规模化落地远程 MCP 时最痛的一环。

**机制**（标识符 `io.modelcontextprotocol/enterprise-managed-authorization`，2026 年 6 月随本规范周期转 stable，Okta 为首个落地的身份提供商）：

```
[ 员工 Agent 登录企业 SSO ]
            │
            ▼
[ 身份提供商 IdP ] 按组织策略（用户组/角色/条件访问）评估该员工可用哪些 MCP Server
            │  签发 ID-JAG（Identity Assertion JWT Authorization Grant，身份断言授权）
            ▼
[ MCP Client 持 ID-JAG 直连目标 MCP Server ] ──► Server 用它向 IdP 兑换访问 Token
            │
            ▼
   员工零弹窗、零感知完成接入；权限变更/回收由 IT 管理员在 IdP 侧集中操作
```

三个关键认知：

1. **决策点上移**：授权从“员工逐次点同意”变成“IT 管理员在 IdP 里一次性配置连接器策略”，员工首次登录自动继承权限。这正是 6.4 节两种免密方案的组织级进化形态——SSO 静默授权被标准化成了跨厂商协议。
2. **EMA 只管“准入”，不管“动作”**：它回答的是“这个员工的 Agent 允许连接哪些 Server”，连接之后每次工具调用的细粒度授权（能调哪个工具、传什么参数）仍归第六章的服务端授权判断与 8.3 节的工具白名单——两层不要混淆。
3. **与 6.3 节加固包正交互补**：CIMD 解决“客户端是谁”的注册问题，EMA 解决“组织允许谁用谁”的策略问题，二者共同构成企业级身份拼图。

#### 三个官方扩展的协同关系

```
              ┌──────────────── 企业身份平面 ────────────────┐
              │   EMA：组织级准入（谁能连哪个 Server）        │
              └───────────────────┬─────────────────────────┘
                                  ▼
[ 核心协议：无状态 tools/call / resources/read / prompts/get ]
              │                                   │
              ▼                                   ▼
     Tasks 扩展：调用“算不完”时            Apps 扩展：结果“说不清”时
     异步化（句柄+轮询+订阅）              可视化（ui:// 模板+沙箱 iframe）
```

一句话概括三者的分工：**EMA 管“进不进得来”，Tasks 管“等不等得起”，Apps 管“看不看得懂”**。三者都不改核心协议一字，全部经能力协商按需启用——这正是扩展框架“一等公民”设计的威力：协议核心可以极小，生态能力可以极大。
...
为了全面适配 2026-07-28 无状态规范，MCP Python SDK 进行了底层重构并推出 V2 版本。本章先看变更全貌，再看代码实战，然后看兼容性设计，最后落在工程细节与生态实测（2.5 节）。
...
- **OpenTelemetry 默认开启**：SDK V2 内置 OTel tracing 且默认启用，Trace/Span 自动覆盖每次工具调用的完整链路。接入企业 APM 几乎零成本（呼应 5.5 节“OTel 替代 Logging”的治理方向）。
...
- **升级冒烟测试清单**（社区实践沉淀）：从 v1 升级后至少验证——(1) 双纪元客户端各连一次；(2) `server/discover` 返回的 capabilities 与代码声明一致；(3) 超过 4 MiB 的请求得到 413 而非连接重置；(4) OTel trace 在企业 APM 中可见；(5) MRTR 交互在不支持弹窗的客户端上按预期降级（捕获 `MCPError`，见 5.4 节）。
...
| `destructiveHint` | 可能造成不可逆破坏（删除、覆盖） | 强制人工二次确认（联动 5.6 节 Elicitation / 8.3 节 `require_human_confirmation`） |
...
### 3.6 三原语协同实战：Skill / Sub-Agent 如何同时驾驭三者

至此三原语是分开讲的，但真实世界里它们几乎从不单独出现——**一个 Skill / Sub-Agent 就是三原语的一次协同编排**。本节用一个完整案例把它们拧成一股绳。

#### 角色分工：三原语分别回答 Skill 的三个问题

| Skill 运行要素 | 对应原语 | 回答的问题 | 8.3 节 Manifest 中的字段 |
| :--- | :--- | :--- | :--- |
| 专家 SOP / 人设 / 流程 | **Prompt** | “按什么流程做” | `system_prompt`（`prompts://` 引用，经 `prompts/get` 拉取） |
| 预载只读上下文 | **Resource** | “看着什么做”（0 推理延时挂载） | `attached_resources` / `auto_subscriptions` |
| 可调用的行动能力 | **Tool** | “用什么做”（白名单最小授权） | `allowed_tools` |

#### 运行全过程：以“数据库慢查询巡检 Sub-Agent”为例

主 Agent 收到“帮我巡检一下订单库昨晚的慢查询”后，拉起一个专职 Sub-Agent，三原语的介入时点如下：

```
[ 阶段 0：装配（Harness，0 轮推理） ]
  Prompt 原语  → 注入巡检 SOP 系统提示（先看基线 → 再抓 Top N → 定位全表扫描 → 给优化建议）
  Resource 原语 → attached_resources 预载：db://orders/slowlog/baseline（历史基线）、
                  db://orders/schema（表结构）——直接进入上下文，无需模型开口要
  Tool 原语    → allowed_tools 白名单注入 3 个只读工具 Schema：
                  get_slow_queries / explain_plan / get_table_stats

[ 阶段 1：执行（Sub-Agent 推理循环，N 轮推理） ]
  LLM 按 SOP 依次发起 tools/call：
    get_slow_queries(since="last_night")      ──► 拿到 37 条慢查询
    explain_plan(sql=...)                     ──► 逐个分析执行计划（Tool 驱动，模型自主决策）
  期间若发现需要“重建索引”这类写操作：
    该工具不在白名单 → 直接不可见（Tool Scoping 硬隔离）
    或命中 require_human_confirmation → Elicitation 弹窗等人批准（5.6 节）

[ 阶段 2：收口（返回主 Agent） ]
  Sub-Agent 输出结构化巡检报告（仅结论进主上下文，几十次工具调用的中间过程被隔离）
  若挂载着实时面板，可经 MCP Apps 渲染慢查询趋势图（Resource/ui:// 界面承载）
```

#### 为什么需要 Sub-Agent 这个“隔离容器”？

三原语的协同本身可以在主 Agent 里完成，Sub-Agent 的增量价值在于三点：

1. **上下文隔离**：巡检过程产生 37 条慢查询、几十次 `explain_plan` 的海量中间输出，全部留在 Sub-Agent 的独立上下文里，主 Agent 只收到最终报告——主对话的 Context Window 不被过程数据污染（这是比 8.2 节 Tool RAG 更彻底的上下文治理）；
2. **权限收敛**：Sub-Agent 可以拥有与主 Agent 不同的工具白名单——主 Agent 可能有写库工具，巡检 Sub-Agent 只给只读工具，爆炸半径按任务粒度切割；
3. **独立并行**：多个 Sub-Agent 各自携带自己的 Prompt + Resource + Tool 组合并发执行（如同时巡检订单库、用户库），互不串味。

#### 设计启示

**Prompt 定流程、Resource 备弹药、Tool 给手脚，Sub-Agent 做隔离**——设计一个 Skill 时按这四问自检：SOP 写进 Prompt 了吗？能预载的数据是否都走了 Resource（而非让模型花轮次去查）？工具白名单是否最小且只读优先？中间过程是否需要 Sub-Agent 隔离？8.3 节的声明式 Manifest 正是这四问的配置化答案。
...
## 第五章 V2 废弃特性、能力协商与现代化替代方案

本章分四段：**5.1** 总览废弃政策与三大废弃特性；**5.2** 补齐贯穿全章的能力协商基础 `clientCapabilities`（含常用能力键速查表）；**5.3 ~ 5.5** 逐一拆解 Roots / Sampling / Logging 的现代化替代方案；**5.6 ~ 5.7** 讲透与之配套的两个新机制——结构化 UI 交互三分法与多路复用订阅通道。
...
被废弃的三大特性为：`Roots`、`Sampling` 与 `Logging`。它们的共同死因是：**都依赖“连接存续期间服务端可以向客户端反向发问/推送”的有状态模型**，与无状态核心根本冲突。下面 5.3 ~ 5.5 节逐一拆解三大废弃特性的替代逻辑；5.2 节先补齐贯穿全章的能力协商基础 `clientCapabilities`——理解它，后面的降级路径、交互开关才都有落点。

---

### 5.2 系统化梳理 `clientCapabilities`：声明什么、在哪携带、如何消费

`clientCapabilities` 是贯穿本章所有替代方案的暗线（MRTR 降级、Apps 协商、experimental 通道都要靠它），本节先把它作为一等概念系统化。

#### 1. 双向能力模型：能力协商在 v2 里是不对称的两个方向

| 方向 | 载体 | 时机 | 可缓存性 |
| :--- | :--- | :--- | :--- |
| **服务端 → 客户端**（Server 能做什么） | `server/discover` 响应的 `capabilities`（1.5 节） | 客户端按需主动探测 | 可缓存（`Cache-Control` / TTL），服务端能力相对稳定 |
| **客户端 → 服务端**（Client 能配合什么） | **每个请求** `_meta` 里的 `clientCapabilities` | 随请求强制携带，无“协商动作” | 不可缓存——服务端必须每请求现场读，这正是不对称的根源 |

v1 里两个方向都在 `initialize` 握手时一次性交换；v2 没有握手，于是服务端能力变成“可缓存的按需查询”，客户端能力变成“随请求自包含的声明”。理解这个不对称，`clientCapabilities` 的所有使用纪律都能推导出来。
...
```json
{
  "_meta": {
    "io.modelcontextprotocol/protocolVersion": "2026-07-28",
    "io.modelcontextprotocol/clientInfo": { "name": "acme-cli", "version": "3.2.0" },
    "io.modelcontextprotocol/clientCapabilities": {
      "elicitation": { "form": true },
      "io.modelcontextprotocol/apps": {},
      "experimental": { "com.acme/voice_input": true }
    }
  }
}
```

- **第 0 层：版本与身份**（`protocolVersion` / `clientInfo`）——`protocolVersion` 决定**报文语义**（无状态纪元还是有状态纪元，Protocol Adapter 据此分流，2.4 节）；`clientInfo` 是身份自述，用于日志与兼容性统计，**绝不能用作能力推断依据**（“name 是某知名客户端就当它支持 X”是典型反模式）。
- **第 1 层：原语消费力**——客户端能如何参与三原语交互，例如是否能呈现 Prompt 选择器、是否处理 `list_changed` 热更新（1.7 节/5.7 节）、是否支持资源订阅。服务端据此决定要不要推送对应通知。
- **第 2 层：交互能力**——以 `elicitation` 为代表：客户端是否能把结构化 Schema 渲染成表单并回传强类型答案。它是 5.4 节 MRTR 能否走通的前置开关：声明了 → 服务端发起表单决议；没声明 → 服务端必须走降级路径（纯文本追问或报错）。
- **第 3 层：扩展与实验能力**——官方扩展（如 Apps、Tasks，1.8 节）以标准键声明；非标特性进 `experimental` 命名空间（5.6 节已完整拆解其协商与生命周期治理）。

#### 3. 常用能力键速查表

`clientCapabilities` 下的键随规范演进持续增删（以 `server/discover` 与官方文档为准），工程中最常打交道的如下：

| 能力键 | 类别 | 客户端声明的含义 | 服务端据此的行为分岔 |
| :--- | :--- | :--- | :--- |
| `elicitation.form` | 交互（现行） | 能把 JSON Schema 渲染成结构化表单/确认弹窗，并回传强类型数据 | 走 5.4 节 MRTR 表单决议 ↔ 降级为纯文本追问或直接报 `MCPError` |
| `elicitation`（旧式布尔声明） | 交互（V1 兼容） | 支持旧纪元服务端反向发起的 elicitation 请求 | 仅 V1 兼容引擎内使用；V2 路径应走 MRTR |
| `sampling` | 遗留（V1 兼容，已废弃） | 允许服务端反向借用客户端 LLM 推理（`sampling/createMessage`） | 仅 V1 兼容路径；新实现**禁止**依赖，见 5.4 节 |
| `roots` / `roots.listChanged` | 遗留（V1 兼容，已废弃） | 允许服务端反向查询客户端工作区根目录 | 仅 V1 兼容路径；新实现用显式参数化，见 5.3 节 |
| `io.modelcontextprotocol/apps` | 官方扩展 | 宿主能渲染 `ui://` 沙箱 iframe 界面（5.6 节） | 返回数据 + UI 挂链 ↔ 降级为纯文本结果 |
| `io.modelcontextprotocol/tasks` | 官方扩展 | 支持任务句柄轮询与任务通知（1.8 节） | 长任务异步化（返回 taskId）↔ 同步等待或拒绝超长任务 |
| 通知处理标志（如对 `list_changed` 的响应能力） | 原语消费力 | 客户端愿意并能够接收、处理热更新通知（5.7 节） | 推送通知 ↔ 不推送（客户端只能 TTL 到期后重拉） |
| `experimental.<域名前缀/特性>` | 实验通道 | 支持某个非标私有特性 | 启用增强路径 ↔ 走标准路径；治理纪律见 5.6 节 |

读表要点：**每一个键都对应服务端代码里的一次 if/else**——声明了走富路径，没声明走降级路径，两条路径业务语义必须等价。

#### 4. 服务端消费纪律（四条军规）

1. **每请求现场读**：无状态架构下，请求可能来自负载均衡后的任何路径，服务端不得把“某客户端支持 X”缓存到会话或内存——能力结论的作用域是**当前这一次请求**；
2. **双路径实现**：能力存在走富路径、不存在走降级路径，两条路径业务语义必须等价（5.6 节的代码示例即此模式）；
3. **未知键静默忽略**：收到不认识的 capability 键直接跳过（前向兼容），同理客户端收到未知的服务端能力也忽略——双侧可独立演进；
4. **版本管语义、能力管特性**：`protocolVersion` 决定“报文怎么解析”，`clientCapabilities` 决定“哪些增强特性可以启用”，二者正交——不要用版本号猜特性，也不要用特性声明猜版本。

#### 5. 客户端声明纪律

如实声明（声明了就要真能兑现，否则服务端走富路径后必然出错）；最小声明（不用的能力不要挂，减少服务端误判面）；experimental 键必须带自有域名前缀，避免与未来官方键撞名（5.6 节生命周期治理）。

---

### 5.3 显式参数化 (Explicit Parameterization) 替代 `Roots`
...
### 5.4 多轮交互请求 (MRTR) 与 `Resolve[T]` 依赖注入替代 `Sampling`
...
3. **能力可探测**：客户端是否支持表单交互声明在 `_meta` 的 `clientCapabilities` 中（5.2 节系统化梳理），服务端可提前决定走 MRTR 还是降级路径；这也是下面代码必须捕获 `MCPError` 的协议背景。
...
### 5.5 云原生 OpenTelemetry (OTel) 替代 `Logging`
...
### 5.6 结构化 UI 交互三分法：Elicitation、MCP Apps 与 Generative UI

“让 AI 应用弹出交互界面”实际上是**三种**机制，经常被混为一谈。按归属划分：**Elicitation 与 MCP Apps 都是 MCP 协议族成员**（前者在核心协议，后者是官方扩展），**Generative UI 则是 Harness 框架层的私有机制**，与 MCP 无关：

| 维度 | **Elicitation**（MCP 核心协议） | **MCP Apps**（MCP 官方扩展，SEP-1865） | **Generative UI**（Harness 私有机制） |
| :--- | :--- | :--- | :--- |
| **发起主体** | MCP Server 代码（执行中途反问） | MCP Server 代码（随工具预交付界面） | LLM 大模型（决定输出 UI 描述） |
| **UI 来源** | 客户端按 Schema 现场渲染（Server 只给结构） | Server 预交付完整 HTML 应用（`ui://` 模板） | Harness 组件库按模型输出的描述符渲染 |
| **交互形式** | 模态弹窗、结构化 Form、Approve/Reject 二次确认 | 常驻沙箱 iframe：看板、图表、可交互应用 | 聊天流卡片、动态图表、Canvas |
| **数据回流** | 强类型结构化数据，直接进工具执行 | 经 `updateModelContext` / `ui/message` 回模型上下文 | 交互事件序列化为用户消息回对话流 |
| **核心优势** | 防范理解偏差、结果可编程消费、绝对安全 | 表达力最强、双向通信、Server 完全掌控体验 | 零 Server 改造、契合自然语言上下文流 |
| **典型场景** | 危险操作确认、部署参数表单、凭据输入 | 数据看板、可视化编辑器、复杂多步操作 | 探索性分析、一次性图表与报告呈现 |

5.4 节的 `Resolve[T]` 就是 Elicitation 在 SDK 层的封装。工程选型原则：**要“确定性答案”（是/否、选哪个、填什么参数）走 Elicitation；要 Server 主导的“持续交互应用”（看板、编辑器）走 MCP Apps；要模型主导的“即时富媒体呈现”（图表、报告）走 Generative UI**。

> **附：客户端里“模型弹出三个方案让你选、选完继续生成”是怎么实现的？**
> 这是第四种常见形态：**模型经 Harness 内置问询工具发起的结构化选择**。它不属于 MCP，实现完全在 Harness 内部：
>
> ```
> [ 1 ] LLM 推理中决定“此处需要人类决策”，发起一次内置工具调用
>       （如 AskUserQuestion，参数即问题与选项列表——它的 inputSchema 由 Harness 预定义）
> [ 2 ] Harness 拦截该调用，不交给任何 MCP Server，而是直接在前端渲染选项卡片并阻塞等待
> [ 3 ] 用户点选后，选择结果被包装成该工具的 tool result 返回给模型
> [ 4 ] 模型把“用户的选择”当作普通工具结果读入上下文，据此继续生成
> ```
>
> 本质上是 **Elicitation 的镜像**：Elicitation 是 *Server 代码*发起、经 MRTR 反问；内置问询工具是 *模型*发起、Harness 内置响应。两者的数据通路相同（结构化 Schema → UI → 强类型答案回流），区别只在发起方与执行方都位于 Harness 信任域内，因此不需要任何协议支持。判断一个交互属于哪类，只需问一句：**这个界面是谁发起的、渲染它的 Schema/模板是谁给的？**


#### Generative UI 实现逻辑（一）：LLM 描述符路线（Harness 侧渲染）

这是“Generative UI”一词最原始的形态，完全发生在 **Harness 框架层**，不需要 MCP 协议任何配合，实现链路分四步：

```
[ 1. 组件目录注入 ] Harness 在系统 Prompt 中注入一份“组件描述 Schema”
                    （如 {"type":"bar_chart","props":{"data":[],"xAxis":""}} 的可用组件清单）
[ 2. 模型产出描述 ] LLM 根据工具返回的结构化数据，输出一段符合该 Schema 的 UI 描述 JSON
                    （通常走结构化输出/约束解码，保证 JSON 合法）
[ 3. 宿主校验映射 ] Harness 用 Schema 校验描述 JSON，逐字段映射到自己的
                    原生组件库（React/Vue 组件），未知组件类型直接拒绝或降级为文本
[ 4. 事件回流对话 ] 用户在组件上的交互（点击、筛选）被序列化为一条普通用户消息，
                    回到对话流中驱动下一轮推理
```

这条路线的本质分工：**LLM 只做“布局决策”（用什么组件、绑什么数据），组件的真实代码与样式全部归 Harness 所有**。因此它天然规避了“模型生成 HTML/JS 被注入”的风险——模型输出的只是数据化的组件调用，Harness 对每个字段校验后才渲染。代价是表达力受限于组件目录的丰富度，且每换一种 UI 风格就要扩展一次 Schema。

#### Generative UI 实现逻辑（二）：MCP Apps 扩展路线（Server 自带 UI）

2026-07-28 规范把社区项目 MCP-UI（由 MCP-UI 团队、OpenAI、Anthropic 联合推进）正式收编为**首个官方扩展 MCP Apps（SEP-1865）**，让 MCP Server 可以随工具一起交付自己的交互界面。实现逻辑拆成“声明 → 渲染 → 通信”三段：

**① 声明：UI 即 Resource，工具用 `_meta` 挂链**

```python
# 服务端：把 HTML/JS 界面注册为 ui:// 协议的 Resource
@mcp.resource("ui://sales-dashboard/app.html", mime_type="text/html")
async def dashboard_ui() -> str:
    return load_asset("dashboard.html")   # 界面模板是预先声明的静态资源

# 工具通过 _meta.ui.resourceUri 声明“我的结果配这个界面”
@mcp.tool(meta={"ui": {"resourceUri": "ui://sales-dashboard/app.html"}})
async def query_sales(quarter: str) -> dict:
    return await db.fetch_sales(quarter)   # 返回结构化数据，由界面负责呈现
```

**② 渲染：宿主拉取模板，沙箱 iframe 内联呈现**

工具被调用时，宿主（Harness 前端）发现 `meta.ui.resourceUri`，经 `resources/read` 拉取 `ui://` 对应的 HTML 模板，在聊天流中以**沙箱化 iframe**（受限权限、禁用同源访问）内联渲染。界面拿到工具返回的结构化数据后自行绘制图表/看板/表单。注意模板是**预先声明、可被宿主审核**的静态资源——宿主可以在接入时逐份审查 HTML，而不是运行时信任任意生成物。

**③ 通信：JSON-RPC over postMessage，宿主是唯一网关**

iframe 内的界面**不允许直接访问网络**，它与外界的一切交互都通过 `postMessage` 承载的 JSON-RPC，由宿主中继转发：
...
// 界面反向调用服务端工具（如钻取明细），经宿主中继、复用其授权链
const detail = await app.callServerTool({ name: "fetch_details", arguments: { id: "123" } });

// 把用户在界面上的选择写回模型上下文（界面 → 对话）
await app.updateModelContext({
  content: [{ type: "text", text: "User selected option B" }],
});
// 也可以经 ui/message 直接追问，驱动对话继续
```

这套设计里宿主握有三个控制权：**网络中继权**（iframe 的一切请求经宿主发出，可鉴权可拦截）、**上下文写入权**（`updateModelContext` 决定界面能向模型“说”什么）、**生命周期权**（iframe 的创建、销毁、尺寸全归宿主）。

**降级策略**：客户端不支持 UI 渲染时，工具退化为纯文本结果——这也是为什么 UI 必须以扩展而非核心协议的形式存在（见下文 `experimental` 协商）。

**变体：Remote DOM 渲染**。MCP-UI 社区还验证过另一条路线：UI 逻辑仍在 iframe 沙箱中执行，但**不直接产出 HTML**，而是把 DOM 变更操作序列化为 JSON 消息流（`RemoteReceiver` 接收），由宿主映射到自己的组件库完成真实渲染。沙箱里跑逻辑、宿主侧长像素，安全性与风格一致性更强，代价是表达力受宿主组件库约束——与描述符路线殊途同归。

#### Generative UI 的安全模型

无论哪条路线，三条红线不变：

1. **界面内容视同不可信输入**：`ui://` 模板、工具返回的数据都可能携带注入载荷（9.11 节的投毒模型同样适用于 UI 层），iframe 沙箱 + CSP（`script-src 'self'` / nonce）是硬约束；
2. **iframe 无独立身份**：界面发起的一切调用都复用宿主的用户身份与授权链，不存在“界面向服务端证明自己是某个用户”的通道——这把 6.1 节的隔离原则延伸到了 UI 层；
3. **预声明模板可审计**：宿主应在 Server 接入评审时审查全部 `ui://` 模板，运行期模板变更视同工具描述变更，触发 9.11 节的哈希比对与重新审批。

#### `experimental` 实验性能力通道：声明、协商与降级的实现逻辑

前面多次出现的 `clientCapabilities.experimental`（1.5 节 `_meta` 示例中的 `custom_ui: true`）是 MCP 的**实验性能力通道**，它的实现逻辑值得单拿出来讲透，因为无状态化之后它的语义发生了变化。

**它是什么**：协议核心尚未标准化的能力，以 `experimental` 命名空间下的键值对声明（key 建议使用反域名前缀，如 `"com.company/voice_input": true`）。客户端用它告诉服务端“我支持哪些非标特性”，服务端据此决定是否启用对应的增强行为。

**无状态时代的协商语义**：v1 里能力协商发生在 `initialize` 握手时，一次声明、Session 内有效；v2 没有握手，**能力声明必须随每个请求经 `_meta` 自包含携带**（1.5 节），服务端不得“记住”任何客户端的能力。这带来两个实现约束：

```python
@mcp.tool()
async def query_sales(quarter: str, ctx: Context) -> dict:
    data = await db.fetch_sales(quarter)

    # 每个请求现场检查能力，而不是缓存“该客户端支持 UI”的结论
    caps = ctx.client_capabilities          # 来自本请求 _meta 的解析结果
    if caps.experimental.get("io.modelcontextprotocol/apps"):
        # 富路径：返回数据 + UI 资源挂链，由宿主渲染交互界面
        return {"data": data, "ui": {"resourceUri": "ui://sales-dashboard/app.html"}}

    # 降级路径：纯文本表格，任何客户端都能消费
    return {"data": data, "text": render_ascii_table(data)}
```

1. **服务端：每请求现场判定，双路径实现**。富路径与降级路径必须同时存在且结果等价——同一个工具被新旧两代客户端调用都应得到正确响应（与 2.4 节 Protocol Adapter 的双纪元思想一致）。
2. **客户端：如实声明、宽容忽略**。只声明真正实现的能力；收到不认识的 `experimental` 键时静默忽略（前向兼容），反之服务端收到未知键同样忽略——实验通道因此可以双侧独立演进而不破坏互操作。

**生命周期治理**：experimental 是能力的“孵化器”，标准演进路径是 `experimental 键 → 官方扩展（Extensions）→ 协议核心`。MCP Apps 本身就是完整走过这条路径的样板：从社区 MCP-UI 项目的实验性约定，到 SEP-1865 成为 2026-07-28 的首个官方扩展。对应的企业治理含义：

- experimental 能力**没有稳定性承诺**（不受 2.4 节 12 个月废弃政策保护），生产依赖前必须用特性开关隔离，并做好随时下线的预案；
- 企业网关应将允许使用的 experimental 命名空间列入白名单（配合 1.6 节的头部路由做计量与拦截），防止团队隐式依赖某个随时会变的私有实验键；
- 一旦能力升格为官方扩展（如 Apps），及时从 `experimental` 键迁移到扩展的标准能力声明，并清理双路径中的旧分支。

#### 认知澄清（一）：Elicitation 与 Apps 不是“简易 vs 丰富”，而是五种维度都不同

把两者放在交互复杂度的一根轴上（“Elicitation 简易、Apps 丰富”）只看到了表象，完整区分如下：

| 维度 | Elicitation（含 MRTR/`Resolve[T]`） | MCP Apps |
| :--- | :--- | :--- |
| **交互目的** | 工具执行中途“缺参数/缺确认”的补救问询——先有任务，交互为任务续命 | 结果的呈现与持续操作载体——交互本身就是目的 |
| **数据回流** | 强类型结构化数据，直接进工具执行（可编程消费） | 经 `updateModelContext` 写回模型上下文，由模型理解后续走对话 |
| **生命周期** | 一次性模态，用户作答即销毁 | 常驻 iframe，多轮互动、可反复回调服务端工具 |
| **UI 来源** | 客户端按 Schema **现场渲染**（Server 只给结构） | Server **预交付完整 HTML 应用**（`ui://` 模板） |
| **协议层级** | 核心协议内（MRTR 替代 Sampling 而来） | 可选官方扩展，需宿主声明支持 |

一句话：**Elicitation 是“任务驱动的结构化问询”，Apps 是“界面驱动的持续交互”**。

#### 认知澄清（二）：描述符路线是“私有协议”而非 MCP——兼谈内置工具的同构性

一个敏锐的观察是：描述符路线（本节前文路线一）里，组件目录 Schema 之于渲染引擎，与工具的 `inputSchema` 之于执行器，**模式完全相同**——都是“Schema 契约约束模型输出，确定性引擎消费结果”。同理，Harness 的内置能力（文件增删改查、执行命令）与 stdio/HTTP MCP Server 在执行语义上也同构：都是“宿主拉起一个执行体完成一次调用”。MCP 自身设计就承认了这种同构：传输层抽象 + `Client.from_server` 内存直连（2.3 节）证明“进程内函数、本地子进程、远程服务”只是同一语义的三种部署形态。

但**模式相同 ≠ 都是 MCP**。判定是不是 MCP 的三条试金石：

1. 是否走 **JSON-RPC 2.0** 标准报文（1.1 节的三帧模型）；
2. 是否有**能力协商与服务发现**（`server/discover` / capabilities / `_meta`）；
3. 能否被**任意合规客户端跨厂商复用**。

描述符契约是 Harness 与模型之间的私有约定（A2UI、AG-UI、各家 Generative UI 方案互不兼容），不跨进程、不跨组织；内置工具同厂商、同信任域，也不需要发现与鉴权。**MCP 存在的理由从来不是“能调工具”，而是跨组织边界的互操作与安全治理**——执行体一旦变成第三方 Server，服务发现、能力协商、OAuth（第六章）、审计、投毒防护（9.11 节）才成为刚需。这也正是很多宿主把内置能力直接实现为进程内 MCP Server 的原因：对模型而言统一只有“工具”一个概念，对架构而言跨不跨信任边界一目了然（呼应 9.8 节的分层纪律）。

---

### 5.7 订阅机制重构：`subscriptions/listen` 单流多路复用 (Multiplexing)
...
企业里 MCP 服务很少独立占一个端口，更常见的形态是**作为子应用挂载进现成的 FastAPI / Starlette 主服务**（与业务 REST API 同进程、同网关）。这个场景下有五个极易踩中的底层死坑（Lifespan 不触发、TaskGroup 同任务进出、307 裸路径重定向、421 DNS 重绑定、ContextVar 泄漏），本章逐一拆解原理并给出可抄的生产级代码。
...
2. **管理推送通道**：维持 5.7 节那条 `/subscriptions/listen` 多路复用响应流，以及断流重连后的事件恢复（event replay）；
...
【2. Agent 框架层 (Harness / Framework Runtime - 智能体引擎)】
 职责：
   ...
   - 自动补全上下文参数（如自动注入 workspace_dir，见 5.3 节）
...
分层的价值在于**职责单向流动**：UI 层不懂协议细节，Harness 层不渲染像素，协议层不做推理决策。前文反复强调的“状态上移到 Harness”（5.3 节的参数补全、6.5 节的 Token 管理）、第九章的“MCP Server 只是被动能力提供者”，都是这张图的推论。
...
这与 3.1 节 Resource 的 URI 参数化是同一哲学（按需取、不预载），只是一个作用于数据、一个作用于工具。配合 5.7 节的 `tools/list_changed` 热更新，工具集的增删对运行时完全透明。
...
# 3. 绑定的系统人设与工作流（第三章 Prompts 原语）
#    值是对 MCP Prompt 的引用：Harness 解析后向 k8s_mcp_server 发起
#    prompts/get(name="sre_incident_investigation")，拉取 Server 侧统一托管、
#    版本化的 SOP（配合 prompts/list_changed 热更新），作为本 Skill 的系统提示；
#    另留一份内联兜底文本，MCP Server 不可达时降级使用。
system_prompt: "prompts://k8s_mcp_server/sre_incident_investigation"
...
# 5. 预先绑定挂载的数据资源 (Attached Resources)：0 推理延时挂载（3.3 节模式 B）
#    Agent 启动即携带集群状态与 SOP 手册，无需模型再花轮次取数
attached_resources:
  - "k8s://cluster/production/status"
  - "file:///docs/ops/sop_handbook.md"

# 6. 自动监听的资源 (Auto Subscriptions)：走 5.7 节的多路复用订阅流，
...
#    白名单之外单独开口子——允许重启，但必须经 Elicitation 确认弹窗（5.6 节），
...
逐字段回看，这份 YAML 没有发明任何新概念——它只是把第三章的原语、第五章的交互（subscriptions/Elicitation 确认）、第八章的架构（Harness 的工具白名单与模型配置）以声明方式钉死在版本库里。尤其值得注意的是：**引入 `system_prompt` 的 `prompts://` 引用之后，这份 Manifest 把 MCP 三大原语全部以线上方式消费了一遍**——`prompts/get` 取流程（Prompt）、`resources/read` 挂数据（Resource）、`tools/call` 做动作（Tool），三原语在同一个 Skill 里闭环（运行时的协同编排详见 3.6 节）。

> 一点精确性说明：`prompts://k8s_mcp_server/sre_incident_investigation` 是 **Harness 层面的引用约定**（意为“向 `k8s_mcp_server` 发起 `prompts/get`，取名为 `sre_incident_investigation` 的 Prompt”）。MCP 协议本身只为 Resource 定义 URI，Prompt 按**名称**寻址、没有官方 URI scheme——各家 Harness 的引用写法不尽相同，阅读其他产品的 Manifest 时留意这一差异即可。

**诊断 Agent 的行为边界（能看什么、能做什么、按什么流程做、什么必须问人）全部可审计**，这就是 Agent as Code 的核心价值。
...
- **避坑指南**：云端产物文件写入 S3/OSS 并返回公网 Resource URI（以 Resource 原语回传，而非本地路径）；长任务改用 **Tasks 扩展的异步提交 + 轮询/订阅范式**（1.8 节）：先返回任务句柄，客户端经 `tasks/get` 轮询或等 `subscriptions/listen`（5.7 节）的完成通知，而不是硬扛同步请求。
...
- **阶段 2（单点生产）**：`stateless_http=True` 上 K8s（4.5 节思路 1）；OAuth 2.1 全链路打通（第六章）；工具白名单 + 破坏性操作确认链路（8.3 节模式）；OTel 接入既有 APM（5.5 节）。核心产出是**第一个达到生产安全基线的 Server**。
...
3. **三大原语是能力抽象的稳定内核**——Tools / Resources / Prompts 的三分法与工具注解（第三章）不因传输纪元更迭而改变，并在 Skill / Sub-Agent 中完成“流程、数据、动作”的协同闭环（3.6、8.3 节）；
4. **无状态不等于无能**——`_meta` 自包含、`Mcp-Method` 头路由、可缓存列表、显式状态句柄、Tasks 扩展（第一章）证明：无状态核心之上，交互、推送、长任务、业务状态一样都不少；
5. **安全是体系而非开关**——凭据全程不碰 LLM 上下文（6.1 节），OAuth 2.1 全家桶（RFC 9728/8707/9207/8693 + PKCE + CIMD）在 Harness 层闭环，混淆代理人、令牌透传、SSRF、工具投毒各有明确的官方防线（第六章、9.11 节）；
6. **工程落地收拢于架构与治理**——三层架构厘清职责边界（第八章），ASGI 挂载五坑保生产平安（第七、九章），声明式 Manifest 把一切钉成可审计的代码（第八章），四阶段路线图与检查清单让落地有章可循（第十章）。
