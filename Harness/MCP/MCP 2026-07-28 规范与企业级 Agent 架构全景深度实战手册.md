...
设计要点：`_meta` 的 key 采用 `io.modelcontextprotocol/xxx` 的反域名命名空间，避免与业务参数撞名，也为第三方扩展预留了空间（自定义 key 使用自己的域名前缀即可）。其中 `clientCapabilities.experimental` 是非标实验特性的声明通道，上例的 `custom_ui` 即属此类——它的协商、降级与生命周期治理在 5.5 节有完整拆解。v1 中靠握手协商一次、Session 内隐式生效的信息，现在被显式摊平到每个请求上——这正是“请求自包含”的物理实现。
...
- **可选协商，非强制**：扩展不进入协议核心，客户端/服务端通过能力声明（`_meta` 的 capabilities，参见 5.5 节 experimental 通道的演进路径）按需协商启用，不支持的端照常工作——**核心小而不失功能，扩展强而不绑架核心**；
...
**机制**：Tasks 从实验核心移出、以 `io.modelcontextprotocol/tasks` 扩展正式发布——工具调用可以返回一个**任务句柄**而非最终结果，客户端通过轮询式 `tasks/get` 查询进度，服务端经 `tasks/update` 推送增量状态；任务变更通知不再走旧的 HTTP GET 端点，而是复用 5.6 节那条 `subscriptions/listen` 单流（客户端按通知类型订阅）。
...
**机制**：Server 将 HTML/JS 界面注册为 `ui://` 协议的 Resource，工具经 `meta.ui.resourceUri` 挂链；宿主在沙箱 iframe 中渲染，界面经 `postMessage` + JSON-RPC 由宿主中继与 Server 双向通信。它由社区 MCP-UI 项目联合 OpenAI、Anthropic 共同推进，是**历史上第一个从 experimental 通道走完“实验 → 官方扩展”完整路径的样板**。声明、渲染、通信三段实现逻辑与安全模型已在 5.5 节完整拆解，此处不再重复。
...
前面多次出现的 `clientCapabilities.experimental`（1.4 节示例中的 `custom_ui: true`）是 MCP 的**实验性能力通道**，它的实现逻辑值得单拿出来讲透，因为无状态化之后它的语义发生了变化。
