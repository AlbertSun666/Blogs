...
| :--- | :--- | :--- | :--- |
| 专家 SOP / 人设 / 流程 | **Prompt** | “按什么流程做” | `prompt.source`（经 `prompts/get` 拉取） |
...
#    两种来源二选一：
#    方式一（推荐）：经 MCP Prompt 原语远程获取——Harness 对 k8s_mcp_server
#      发起 prompts/get，把 Server 侧版本化托管的 SOP 拉取下来作为系统提示。
#      好处：SOP 由领域团队在 Server 侧统一维护、热更新（配合 prompts/list_changed），
#      所有引用该技能的 Agent 无需改配置即同步最新版本；
#    方式二（兜底）：本地内联 system_prompt——MCP Server 不可达时降级使用。
prompt:
  source: "mcp://k8s_mcp_server/prompts/sre_incident_investigation"   # → prompts/get
...
逐字段回看，这份 YAML 没有发明任何新概念——它只是把第三章的原语、第五章的交互（subscriptions/Elicitation 确认）、第八章的架构（Harness 的工具白名单与模型配置）以声明方式钉死在版本库里。尤其值得注意的是：**引入 `prompt.source` 之后，这份 Manifest 把 MCP 三大原语全部以线上方式消费了一遍**——`prompts/get` 取流程（Prompt）、`resources/read` 挂数据（Resource）、`tools/call` 做动作（Tool），三原语在同一个 Skill 里闭环（运行时的协同编排详见 3.6 节）。**诊断 Agent 的行为边界（能看什么、能做什么、按什么流程做、什么必须问人）全部可审计**，这就是 Agent as Code 的核心价值。
